...
export function EncounterCreator({ setActiveTool }: EncounterCreatorProps) {
  ...
  const handleSaveParty = () => {
    ...
    if (loadedPartyId) {
      const updated = savedParties.map((p) =>
        p.id === loadedPartyId
          ? { ...p, name: partyNameInput.trim(), players: [...players] }
          : p
      )
      setSavedParties(updated)
      setOriginalPlayers([...players])
      alert(`Party "${partyNameInput.trim()}" updated!`)
    } else {
  ...
  const handleLoadParty = (party: SavedParty) => {
    ...
    setLoadedPartyId(party.id)
    setOriginalPlayers([...party.players])
                  ...
                  <>
                    <div className="space-y-2">
                      {players.map((player) => {
                        ...
                        const levelChanged = originalPlayer && originalPlayer.level !== player.level
                        return (
                          <div
                            key={player.id}
                          ...
                          >
                            <div className="flex items-center gap-2 flex-1">
                              ...
                              <div className="flex-1">
                                ...
                                <div className={`text-xs ${levelChanged ? "text-blue-600 font-semibold" : "text-gray-500"}`}>
                                  Level {player.level}
                                </div>
                              </div>
                            </div>
